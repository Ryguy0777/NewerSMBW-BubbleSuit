...
#define BubbleSuit WM_TEST
#define BubbleCoin WM_TEST2
...
void dSuitBubble_c::spriteCollision(ActivePhysics *apThis, ActivePhysics *apOther) {
	if (!this->hasTrappedEnemy && this->canBePopped) {
    	...
    	if (name == EN_JUMPDAI || name == WM_TEST || name == AC_BLOCK_GROUP || name == EN_MARUTA || name == EN_BAKUBAKU || name == EN_BIG_TOGETEKKYU || 
		name == EN_RAIL_POLY_ICE || name == LIFT_ZEN_TOGE || name ==  EN_HANACHAN || name == AC_PROP_BLOCK)
    	{if (name != BubbleSuit) {OSReport("Bubble is colliding with %d, so we're ignoring it\n", name);} return; }
		// list of actors that pop
		else if (name == EN_CHOROBON || name == WM_TREASURESHIP || name == EN_BOSS_KOOPA || name == EN_IGA_KURIBO || name == EN_UNIZOO || name == EN_SANBO || name == EN_BOSS_LARRY || 
		name == EN_BOSS_WENDY || name == EN_BOSS_IGGY || name == EN_BOSS_LEMMY || name == EN_BOSS_MORTON || name == EN_BOSS_ROY || name == EN_BOSS_LUDWIG || name == EN_BOSS_KAMECK ||
		name == EN_BOSS_CASTLE_LARRY || name == EN_BOSS_CASTLE_WENDY|| name == EN_BOSS_CASTLE_IGGY || name == EN_BOSS_CASTLE_LEMMY || name == EN_BOSS_CASTLE_MORTON || name == EN_BOSS_CASTLE_ROY || 
		name == EN_BOSS_CASTLE_LUDWIG|| name == EN_JIMEN_PAKKUN || name == EN_JIMEN_BIG_PAKKUN || name == EN_JIMEN_FPAKKUN || name == EN_JIMEN_BIG_PAKKUN|| name ==  EN_IGAPUKU || name == EN_TOGEZO ||
		name == EN_IBARAMUSHI || name == WM_SMALLCLOUD || name == AC_LIFT_ICE_SPRING)
